...
const Order = () => {
  ...
  useEffect(() => {
    const fetchCheckoutData = async () => {
      try {
        const { data } = await axiosSecure.get("/one-click-checkout-data");
        if (data.status) {
  ...
  const handleQtyChange = async (id, actionOrQty) => {
    ...
    try {
      const res = await axiosSecure.put(`/oneclickorder/${id}`, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { data } = await axiosSecure.post("/one-click-checkout-data", payload);
